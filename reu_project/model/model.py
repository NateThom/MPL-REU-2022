...
def model_functions(train_set=CelebA_Dataset, model_file="xyzzy", new_file=None,
    ...
    if train:
        ...
        while len(set(val_loss_record[-3:])) != 1 or epoch_index <= 5:
            ...
            finish = datetime.timedelta(seconds=round(time.time() - start, 2))
    ...
    if save:
        ...
        if train:
            ...
            with open(path_to_saved_models + model_file + '_data.csv', 'w+') as dataFile:
...
model_functions(train_set=CelebA_Dataset, model_file="celeba_t2", train=True, save=True,
                train_split=(0, 0.025), val_split=(0.8, 0.81))
# model_functions(train_set=Occluded_Dataset, model_file="occluded_v1", train=True, save=True)
...
# optimizer's learning rate has been reduced 100 times after testing.
