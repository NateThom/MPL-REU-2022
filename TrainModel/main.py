...
import csv
from load_data import CelebADataset, AugmentedDataset
...
def train_loop(model, train_loader, val_loader, epoch, device):
    ...
    for batch_index, batch in enumerate(train_loader):
        ...
        # update loading bar
        tick_count = progress((batch_index + 1) / len(train_loader), tick_count)
    ...
    with torch.no_grad():
        ...
        for batch_index, batch in enumerate(val_loader):
            ...
            # update loading bar
            tick_count = progress((batch_index + 1) / len(val_loader), tick_count)
...
#     form (split_start, split_end), where split_start and split_end are floats ranging from
#     0 to 1.
#
# test_split:
...
#
# ilr:
#     Initial learning rate. The Adam optimizer default learning rate is 0.001, but after
#     testing, I've set this parameter's default to 0.0001.
#
# lrs:
#     Learning rate scheduler. There are currently three implemented LRS: exponential,
