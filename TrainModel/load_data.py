...
#
#         # add image names
#         path = '/home/guest/MPL-REU-2022/swapped_attributes/single/'
#         self.image_names = []
#
...
class AugmentedDataset(Dataset):
    def __init__(self, split, transform=None):
        ...
        # add image names
        path = '/home/guest/MPL-REU-2022/swapped_attributes/double/'
        self.image_names = []

        split_min = floor(split[0] * 202599) + 1
        split_max = floor(split[1] * 202599) + 1

        # find path to current augmentation
        indices_aug = [x for x, y in enumerate(transform[0]) if y != 0]
        folder_combination = ''
        for i in indices_aug:
            folder_combination += self.image_augmentations[i] + '_'
        folder_combination = folder_combination[:-1]
        ...
        for im in range(split_min, split_max):
            ...
            if im in chosen_augmented:
                # get path to directory for image numbered im
                curr_path = path + folder_combination + '/' + str(im) + '/'
                os.chdir(curr_path)
                list_augmented_images = glob.glob(curr_path + '/*.jpg')
...
class CelebADataset(Dataset):
    def __init__(self, split, transform=None):
        ...
        img_gender = [int(v > 0) for v in img_gender]
        self.labels = dict(zip(img_names, img_gender))
        path = '/home/guest/MPL-REU-2022/swapped_attributes/single/normal/'
        self.image_names = []
        split_min = floor(split[0] * 202599) + 1
        split_max = floor(split[1] * 202599) + 1
        for im in range(split_min, split_max):
